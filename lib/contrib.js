...
exports.init = function(grunt) {
  ...
  exports.expandIndividualDests = function(files, root, flatten) {
    return files.reduce(function(expanded, file) {
      var srcFiles = grunt.file.expand(file.src);
      var basePath = exports.findBasePath(srcFiles, root);
      var dest = path.normalize(file.dest);

      if (exports.isIndividualDest(dest)) {
        srcFiles.forEach(function(srcFile) {
          expanded.push({
            dest: exports.buildIndividualDest(dest, srcFile, basePath, flatten),
            src: [ srcFile ]
          });
        });
      } else {
        expanded.push({ dest: dest, src: srcFiles });
      }
  ...
  exports.isDestUpToDate = function(file) {
    try {
      var mtimeDest = fs.statSync(file.dest).mtime;
    } catch (e) {
      return false;
    }

    var srcFiles = grunt.file.expand(file.src);
    return ! srcFiles.some(function(src) {
      return fs.statSync(src).mtime > mtimeDest;
    });
  };
